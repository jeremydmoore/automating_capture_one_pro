...
# ============ variables ============ #
tell_co20 = 'Tell application "Capture One 20"'
...
def get_selected_variants():
    ...
    command = command_stub + [f'{tell_co20} to set selected_variants to (get selected variants)', 'return selected_variants']
...
def set_adjustment_value(document, collection_id, variant_id, adjustment, value, value_is_quoted=True):
    ...
    if value_is_quoted:
        value = f'"{value}"'
    command = command_stub + [f'{tell_co20} to tell its document "{document}" to tell its collection id "{collection_id}" to set {adjustment} of adjustments of variant id "{variant_id}" to {value}']
...
def get_adjustment_values(document, collection_id, variant_id, adjustment):
    ...
    command = command_stub + [f'{tell_co20} to tell its document "{document}" to tell its collection id "{collection_id}" to set adjustment_values to {adjustment} of adjustments of variant id "{variant_id}"', 'return adjustment_values']
...
def get_crop_box(document, collection_id, variant_id):
    ...
    command = command_stub + [f'{tell_co20} to tell its document "{document}" to tell its collection id "{collection_id}" to set crop_box to crop of variant id "{variant_id}"', 'return crop_box']
...
def set_crop_box(document, collection_id, variant_id, crop_box):
    ...
    command = command_stub + [f'set center_x to ({crop_box[0]} as number)',
                              ...
                              f'{tell_co20} to tell its document "{document}" to tell its collection id "{collection_id}" to set crop of variant id "{variant_id}" to crop_box'
                             ]
    applescript.process(command)
...
def get_value(document, collection_id, variant_id, value):
    command = command_stub + [f'{tell_co20} to tell its document "{document}" to tell its collection id "{collection_id}" to set current_value to {value} of variant id "{variant_id}"', 'return current_value']
...
def get_session_directory_path(document, posix=True):
    ...
    command = command_stub + [f'{tell_co20} to tell its document "{document}" to set document_path to {path_format}', 'return document_path']
...
def get_primary_variant():
    command = command_stub + [f'{tell_co20} to set primary_variant to (get primary variant)', 'return primary_variant']
    primary_variant = applescript.command_to_python_list(command)[0]
    return primary_variant

# primary variant functions
def get_primary_variant_id():
    command = command_stub + [f'{tell_co20} to set primary_variant_id to id of primary variant', 'return primary_variant_id']
    primary_variant_id = applescript.command_to_python_list(command)[0]
    return primary_variant_id

def get_primary_variant_document():
    command = command_stub + [f'{tell_co20} to set primary_variant_document to document of primary variant', 'return primary_variant_document']
...
def get_primary_variant_collection_id():
    command = command_stub + [f'{tell_co20} to set primary_variant_collection_id to document of primary variant', 'return primary_variant_document']
...
def set_variant_as_primary(document, collection_id, variant_id):
    command = command_stub + [f'{tell_co20} to set primary variant to variant {variant_id} of collection {collection_id} of document {document}']
    applescript.process(command)
...
class Variant():
    ...
    def __init__(self, selected_variant):
        ...
        command = command_stub + [f'{tell_co20} to tell its document "{self.document}" to tell its collection id "{self.collection_id}" to set image_name to name of variant id "{self.id}"', 'return image_name']
    ...
    def get_layer_names(self):
        layer_list = get_value(self.document, self.collection_id, self.id, 'layers')
        return layer_list
    ...
    def get_layer_names(self):
        ...
        applescript.display_dialog(f'layer_list: {layer_list}')
        #layer_dict = {}
        #
        for layer in layer_list:

            layer_info = layer.split(' ')
            layer_index = layer_info[2]
            # get name of layer
            command = command_stub + [f'{tell_co20} to tell its document "{self.document}" to tell its collection id "{self.collection_id}" to set layer_name to name of layer {layer} of variant id "{self.id}"', 'return layer_name']
            layer_name = applescript.command_to_python_list(command)  # get first item in list
            # applescript.display_dialog(layer_name)
            applescript.display_dialog(f'layer: {layer}\n\nlayer name: {layer_name}')
            #layer_dict.update( {layer_name : layer})

        #applescript.display_dialog(f'layer dict: {layer_dict}')
        #     Reference: layer "Luma range gradient from left" of primary variant
        #     Value: layer 2 of variant id "11826" of collection id "11" of document "athletics-programs.cosessiondb"
        # name of layer "Luma range gradient from left" of primary variant
    ...
    def reset_crop(self):
        ...
        command = command_stub + [f'{tell_co20} to tell its document "{self.document}" to tell its collection id "{self.collection_id}" to set image_dimensions to dimensions of parent image of variant id "{self.id}"']
    ...
    def set_star_rating(self, rating):
        ...
        command = command_stub + [f'{tell_co20} to tell its document "{self.document}" to tell its collection id "{self.collection_id}" to set rating of variant id "{self.id}" to "{rating}"']
        applescript.process(command)
    ...
    def output_with_recipe(self, process_recipe):
        ...
        command = command_stub + [f'{tell_co20} to tell its document "{self.document}" to set recipe_output_folder to ((root folder location of current recipe as text) & (output sub folder of recipe "{process_recipe}"))',
        ...
        if self.expected_output_path.is_file():
            ...
            command = command_stub + [f'{tell_co20} to tell its document "{self.document}" to tell its collection id "{self.collection_id}" to process variant id "{self.id}" recipe "{process_recipe}"']
            applescript.process(command)

            return self.expected_output_path

        # return path of processed image
        #command = command_stub + [f'{tell_co20}'
